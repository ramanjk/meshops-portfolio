"""Interactive chat server for the hello-gateway steward.

Enabled with ``CHAT_ENABLED=true``. Serves a small HTTP API (and a minimal web
UI) so you can talk to the Gateway Steward's persona and exercise its read-only
routing-plane tool (litellm-mcp). This is a long-lived process, so the Deployment
...
gains the single, NON-mutating ``propose_budget`` tool and the gated-write
...
from .write import LiteLLMBudgetApplier, build_propose_budget_tool

LOG = logging.getLogger("meshops.hello-gateway.chat")
...
<title>Gateway Steward — chat</title>
<style>
...
<body>
  <h1>🚦 Gateway Steward — chat</h1>
  <div id="log"></div>
  <form id="f">
    <input id="m" autocomplete="off" placeholder="Ask about routes, budgets, upstream health…" autofocus/>
...
def _friendly_error(exc: Exception) -> str | None:
    ...
    if "contentfilter" in text or "content_filter" in text or "responsible ai" in text:
        return (
            ...
            "human-gated regardless. Ask me about the routing plane and I'll gladly help."
        )
...
def _build_app(settings: Settings) -> FastAPI:
    app = FastAPI(title="Gateway Steward Chat")
    state: dict[str, Any] = {}
    ...
    async def _startup() -> None:  # pragma: no cover - integration path
        ...
        # mutating propose_budget tool. Off = byte-for-byte the read-only steward.
        if settings.write_enabled:
            ...
            LOG.info(
                "[chat] WRITE-ENABLED: HITL gate armed for LiteLLM route budget in "
                "ns/%s via '%s' channel",
                settings.budget_namespace,
                channel.name,
            )
    ...
    async def chat_endpoint(req: ChatRequest) -> ChatReply:
        ...
        with tracer.start_as_current_span(
            "gateway.steward.chat", kind=SpanKind.CLIENT
        ) as span:
