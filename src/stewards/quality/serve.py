...
gains the single, NON-mutating ``propose_annotation`` tool and the gated-write
...
from .write import LangfuseApplier, build_propose_annotation_tool
...
<body>
  <h1>🔬 Quality Steward — chat</h1>
  <div id="log"></div>
  <form id="f">
    <input id="m" autocomplete="off" placeholder="Ask about traces, eval scores, quality drift…" autofocus/>
...
def _build_app(settings: Settings) -> FastAPI:
    ...
    async def _startup() -> None:  # pragma: no cover - integration path
        ...
        # mutating propose_annotation tool. Off = byte-for-byte the read-only steward.
        if settings.write_enabled:
            ...
            LOG.info(
                "[chat] WRITE-ENABLED: HITL gate armed for Langfuse annotations via '%s' channel",
                channel.name,
            )
