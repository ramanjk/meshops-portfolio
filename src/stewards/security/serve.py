"""Interactive chat server for the hello-security steward.

Enabled with ``CHAT_ENABLED=true``. Serves a small HTTP API (and a minimal web
UI) so you can talk to the Security Steward's persona and exercise its read-only
proposal-queue tool (github-sec-mcp). This is a long-lived process, so the
...
gains the single, NON-mutating ``propose_quarantine`` tool and the gated-write
persona; every actuation still passes the HITL gate (ADR-0011) and is a single
allow-listed GitHub label write. The write-path plumbing is shared across
stewards via :mod:`stewards.hitl`.
...
from .write import GitHubLabelApplier, build_propose_quarantine_tool

LOG = logging.getLogger("meshops.hello-security.chat")
...
<title>Security Steward — chat</title>
<style>
...
<body>
  <h1>🛡️ Security Steward — chat</h1>
  <div id="log"></div>
  <form id="f">
    <input id="m" autocomplete="off" placeholder="Ask about open proposals / injection risk…" autofocus/>
...
def _friendly_error(exc: Exception) -> str | None:
    ...
    if "contentfilter" in text or "content_filter" in text or "responsible ai" in text:
        return (
            ...
            "human-gated regardless. Ask me about the proposal queue and I'll gladly help."
        )
...
def _build_app(settings: Settings) -> FastAPI:
    app = FastAPI(title="Security Steward Chat")
    state: dict[str, Any] = {}
    ...
    async def _startup() -> None:  # pragma: no cover - integration path
        ...
        # mutating propose_quarantine tool. Off = byte-for-byte the read-only steward.
    ...
    async def chat_endpoint(req: ChatRequest) -> ChatReply:
        ...
        with tracer.start_as_current_span(
            "security.steward.chat", kind=SpanKind.CLIENT
        ) as span:
