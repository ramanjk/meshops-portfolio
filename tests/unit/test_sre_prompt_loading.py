"""Unit tests for hello-sre prompt loading and empty-file fallback."""
from __future__ import annotations

from pathlib import Path

import stewards.sre.agent as agent


def test_read_prompt_returns_repo_prompt_content() -> None:
    text = agent._read_prompt("sre-steward.chat.md")
    assert "SRE Steward" in text
    assert "non-negotiable" in text.lower()


def test_gated_write_persona_present() -> None:
    text = agent._read_prompt("sre-steward.gated-write.chat.md")
    assert "propose_scale" in text
    assert "HITL gate" in text


def test_empty_in_cluster_prompt_falls_back(tmp_path: Path, monkeypatch) -> None:
    empty = tmp_path / "sre-steward.chat.md"
