"""Unit tests for hello-security prompt loading and empty-file fallback."""
from __future__ import annotations

from pathlib import Path

import stewards.security.agent as agent


def test_read_prompt_returns_repo_prompt_content() -> None:
    text = agent._read_prompt("security-steward.chat.md")
    assert "Security Steward" in text
    assert "non-negotiable" in text.lower()


def test_gated_write_persona_present() -> None:
    text = agent._read_prompt("security-steward.gated-write.chat.md")
    assert "propose_quarantine" in text
    assert "HITL gate" in text


def test_empty_in_cluster_prompt_falls_back(tmp_path: Path, monkeypatch) -> None:
    empty = tmp_path / "security-steward.chat.md"
    ...
    result = agent._read_prompt("security-steward.chat.md")
    assert result.strip(), "empty in-cluster file must fall back to the repo prompt"
    assert "Security Steward" in result
