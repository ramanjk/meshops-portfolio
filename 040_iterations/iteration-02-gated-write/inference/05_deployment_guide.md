...
- **KAITO workspace scaling** would be a `patch` on `spec.resource.count` (operation `patch`), not `kubectl scale`; the gate supports both `patch` and `scale`. **Caveat (KAITO):** on current KAITO the Workspace `resource.count` field is **immutable** — its admission webhook rejects any change with `field is immutable: resource.count`. The gate handles this correctly: the **server dry-run surfaces the error at propose time and the proposal fails closed** (no change even if approved). This is a good *fail-closed* demo, but it is **not** a working approve→act example. For the headline "approve → it actually changes the cluster" demo, use **"create a test pod"** (mutable, RBAC-allowed) — see [TC-W1](03_test_cases_manual.md). Changing a Workspace's replica count in practice means recreating the Workspace, which is out of this slice's scope.
